...
Begin VB.Form frmUPX 
   ...
   Caption         =   "Eexecutable Compressor"
   ClientHeight    =   3165
   ClientLeft      =   45
   ClientTop       =   360
   ClientWidth     =   7590
...
Private sExeName As String 'Full path to the project's executable
...
Public Sub SetProject(sProjectExe As String)
...
Public Sub CompressFile()
    ...
    Else
        ...
        If Dir$(sExePath & "upx.exe") = vbNullString Then
        ...
        If ret = 0 Then
            ...
            If MsgBox("The file has been downsized from " & Format$(FSize(0) / 1024, "#,##0.0") & "K to " & Format$(FSize(1) / 1024, "#,##0.0") & "K (" & Format$((FSize(1) - FSize(0)) / FSize(0), "0.0%") & ")." & vbCrLf & "Do you wish to run the executable now?", vbYesNo + vbInformation, "Success") = vbYes Then
                ShellExecute Me.hwnd, "open", sExeName, vbNullString, vbNullString, 1
            End If
        ElseIf ret = 2 Then
            'error or already compacted (most probably)
...
ExitCompress:
    ...
    Dim sKill As String
    sKill = installPath & "\" & File_Token(sExeName, "f")
    If File_Exists(sKill) = True Then Kill sKill
    FileCopy sExeName, sKill
    Me.Hide
    Unload Me
    Err.Clear
    Exit Sub
...
Public Function ShellAndWait(Filename As String) As Long
    On Error Resume Next
